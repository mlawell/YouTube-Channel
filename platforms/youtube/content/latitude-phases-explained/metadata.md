# Metadata — Latitude Margaritaville Watersound: Every Phase Explained

## Titles

Rule: lead with the search term, not with a clever question. Someone typing
"latitude margaritaville watersound phases" into YouTube has to see their exact
words in the first three or four of the title.

**Primary (use this one):**

```
Latitude Margaritaville Watersound Phases Explained (2026) | All 16, From Someone Who Lives Here
```

**Alternates for A/B:**

```
Latitude Margaritaville Watersound Phases: All 16 Explained by a Resident Realtor
Latitude Margaritaville Watersound Phase Map 2026 | Which Phase Should You Buy In?
Latitude Margaritaville Watersound Phases 1-10 | Which Ones Are Sold Out (2026)
```

Notes:

- Keep "Latitude Margaritaville Watersound" fully spelled out. "LMW" gets no
  search volume.
- "From Someone Who Lives Here" is the differentiator and it goes in the title,
  not just the thumbnail.
- If the primary underperforms in the first 48 hours, swap to the fourth
  option — "which ones are sold out" is the highest-intent phrasing we have.

## Chapters

Paste into the description. Timestamps must be corrected to the final cut —
the first one **must** be `0:00`.

```
0:00 The one thing nobody tells you about the phases
0:12 Why I can answer this (I live in Phase 8)
0:40 Two things everyone gets wrong
1:40 Before you visit the Sales Center — do this first
2:05 The full phase map
3:15 What actually differs between phases
4:45 Phase 1 — the Sales Center & models
5:15 Phase 2
5:45 Phase 3A
6:10 Phase 3B & 3C
6:35 Phase 3D
7:00 Phase 4A
7:30 Phase 4B
8:00 Phase 5A3 — the phase people say was skipped
8:30 Phase 5B
8:50 Phase 5C
9:15 Phase 6A — the lowest density in the community
9:45 Phase 6B & 6C
10:15 Phase 7 — the biggest footprint
10:50 Phase 8 — where I live
11:45 Phase 9
12:05 Phase 10
12:15 The street that's in two phases (Escape Avenue)
14:00 Lot numbers are not addresses
15:00 Phase 10 is the end of Area 1 — and the second Town Square
15:40 Which phase sells next & how long does a build take?
16:20 If you're 2 years out from retiring
17:30 Get the map + how to register with me
```

## Description

First two lines are what shows above the fold. Put the search term and the
authority claim there.

```
Latitude Margaritaville Watersound has 16 recorded phases — and in most of them
the only way in now is a resale. I'm Karen Lawell, I live in Phase 8, and in
this video I walk every single phase on a map I built from Bay County's own
public records.

Two things you'll hear repeated that are simply wrong: Phase 3 isn't one phase
(it's three separate recorded plats — 3A, 3B & 3C, and 3D), and Phase 5A was
never "skipped" (5A3 is a recorded plat, book 32 page 81). Both are checkable
at the Bay County Clerk's office, and I show you the book and page for all 16.

⏱️ ABOUT THE INVENTORY NUMBERS: the resale and homesite counts I give in this
video were true the day I recorded it and they will be different by the time
you watch. That's just how fast this community moves. Everything else here —
the phases, the streets, the addresses, the distances — doesn't change. For
today's actual numbers on any phase, message me and I'll pull them for you.

What this video actually answers:
• Which phases still have new-build homesites and which are resale only
• Why the phases are NOT priced differently — and what really drives price
• The trade-off that decides everything: the Bandshell is the middle of the
  Town Center, so the closer you are, the more you get AND the more you hear
• Highway 79 noise on the eastern edge
• Escape Avenue runs through TWO phases — 9499 and below is Phase 7, 9502 and
  above is Phase 8. Twelve streets here cross a phase line.
• Why Minto's lot numbers are not addresses (there is no 8042 Escape Ave)
• Phase 10 is the end of Area 1 — every one of the 16 plats says "Area 1",
  and Area 2 isn't recorded yet, so nobody can tell you what's next
• What it's honestly like to live in Phase 8, including the trade-offs
• Is there a second Town Square? Which phase releases next? What to do if
  you're two years out from retiring?

📍 WANT THE MAP? Comment "MAP" below or email me and I'll send you the
full-size phase map — all 16 phases, plat book and page for each one, free.

📬 GOT AN ADDRESS? Drop it in the comments and I'll tell you which phase it's
in, which plat it was recorded under, and what's around it.

⚠️ REGISTER BEFORE YOUR FIRST VISIT. Minto pays the buyer's agent commission,
so having me with you costs you nothing — but only if you register with me
BEFORE you walk into the Sales Center. Once you've signed in on your own, I
can't represent you on a new build. Text me first.

📞 Call/Text: 850-517-8528
📧 Email: Karen@nwflbeachhomes.com
📅 Schedule a call: https://karenlawell.countspcb.com/contact.php

Karen Lawell, Realtor | License #3397366 | Brokered by Counts Real Estate Group

Map data: Bay County, Florida recorded plats (public record), plat book and
page shown per phase. Illustrative only — not a survey. For what is actually
for sale today, contact me. This channel is not affiliated with or endorsed by
Minto Communities.

#LatitudeMargaritaville #Watersound #PanamaCityBeach #55PlusCommunity #ActiveAdultLiving
```

## Tags

```
Latitude Margaritaville Watersound
Latitude Margaritaville Watersound phases
Latitude Margaritaville phase map
Latitude Margaritaville Watersound map
Latitude Margaritaville Panama City Beach
Latitude Margaritaville Watersound phase 8
Latitude Margaritaville Watersound phase 9
Latitude Margaritaville Watersound phase 10
Latitude Margaritaville homes for sale
Latitude Margaritaville Watersound review
living in Latitude Margaritaville
Minto Latitude Margaritaville
55 plus communities Florida
active adult community Florida
retiring to Panama City Beach
Watersound Florida
Latitude Margaritaville sold out
Latitude Margaritaville resale
```

## Pinned comment

Post it at publish and pin it immediately — it doubles as the lead magnet
mechanism and the registration warning.

```
Two corrections to what's usually said about the phases, both checkable at the
Bay County Clerk:

1. Phase 3 is THREE recorded plats, not one — 3A (PB 28/34), 3B & 3C (PB 28/63)
   and 3D (PB 30/8).
2. Phase 5A was not skipped. 5A3 is a recorded plat — PB 32/81, 98 homesites.

Want the full-size map? Reply "MAP" and I'll send it.

And please — if you're coming down to see it, text me at 850-517-8528 BEFORE
your first Sales Center visit. Minto pays my commission, so I cost you nothing,
but I can only represent you if you register with me first. — Karen (Phase 8)
```

## End screen (last 20 seconds)

| Slot | Element |
| --- | --- |
| Left | **Subscribe** — Karen's channel icon |
| Right, top | **Best for viewer** — let YouTube pick |
| Right, bottom | Next video: the amenities/Town Center tour (or the model/collection walkthrough once it exists) |

Keep talking over the end screen. Do not go silent — silence is where retention
drops off a cliff.

## Cards

| Time | Card |
| --- | --- |
| ~2:05 | Link to the model & collections video (once published) |
| ~10:50 | Link to a Phase 8 / neighborhood tour |
| ~13:20 | Link to the contact page |

## Publishing settings

- **Category:** People & Blogs
- **License:** Standard YouTube License
- **Audience:** not made for kids
- **Language:** English · **Subtitles:** upload a corrected SRT, don't rely on
  auto-captions — the street names will be mangled
- **Playlist:** add to a "Latitude Margaritaville Watersound — The Community"
  playlist, and make this the first entry
- **Thumbnail:** custom (requires the phone-verified channel — already done)

## Cross-posting

| Platform | Cut |
| --- | --- |
| Facebook group | Full video, native upload, with the map image as the first comment |
| Instagram / TikTok | 45-sec vertical: the "Phase 5A3 was never skipped" correction — it's the most shareable beat |
| Pinterest | The static poster PNG, linked to the video |
| Email list | The poster as a PDF lead magnet |
